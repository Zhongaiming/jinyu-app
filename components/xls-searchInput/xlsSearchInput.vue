<template>
	<view class="searchBox-wrapper" :style="{ background: bgColor }">
        ...
        ">
				<u-icon name="search" color="#8e8e93" size="38"></u-icon>
			</view>
			<view class="cancel-btn" :class="inputFocus ? 'cancel-focus' : ''" @click.stop="cancelOrcomfirm"
	...
	export default {
		...
		methods: {
			...
			inputMethod: debounceFun(async function() {
				if (this.inputEnter) {
					this.$emit("inputSearch", this.inputEnter);
				}
			}, 600),
			...
			cancelOrcomfirm() {
				if (!this.inputEnter) {
					...
					this.inputEnter = "";
					this.$emit("stratesSearch", this.inputEnter);
				} else {
					this.$emit("stratesSearch", this.inputEnter);
				}
			},
