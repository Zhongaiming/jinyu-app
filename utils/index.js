...
const utils = {
	 ...
	 * @param mode datetime为完整时间，date为日期选择，time为时间选择，year-month为年月选择
	 **/
	getDateTimeSlot(type, mode='date') {
		let now = new Date()
	...
	function numberToWords(num) {
	    ...
	    let words = "";
	    let unitIndex = 0;
	    let zeroFlag = false; // 用于判断是否需要添加零
	
	    while (num > 0) {
	        const digit = num % 10;
	
	        if (digit > 0) {
	            words = numbers[digit] + units[unitIndex] + words;
	            zeroFlag = false; // 重置零标志
	        } else if (!zeroFlag) {
	            words = numbers[0] + words; // 添加零
	            zeroFlag = true; // 设置零标志
	        }
	    ...
	    // 移除多余的“零”
	    words = words.replace(/零+/, "零"); // 简化处理，只保留一个零
	    words = words.replace(/零$/, ""); // 去掉末尾的零
