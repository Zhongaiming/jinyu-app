			...
			<view class="wrap-padding">
				...
				<ul class="post-list">
					<li class="item">
						...
						<u-input type="text" v-model="sonAccountInfo.username" placeholder="请输入11位手机号" border="none"
							inputAlign="right" :readonly="disableInput?true:false" @change="inputMethod"/>
					</li>
					<li class="item">
						<span class="item-left">请输入密码</span>
						<u-input type="password" placeholder="新密码" border="none" inputAlign="right"
							v-model="sonAccountInfo.password" />
					</li>
					<li class="item">
						<span class="item-left">使用人姓名</span>
						<u-input placeholder="请输入使用人姓名" border="none" inputAlign="right"
				...
				<ul class="post-list">
					<li class="item arrows-right" @click="$refs.job.showJob(sonAccountInfo.roleId)">
						...
						<span class="item-right" :class="{'font-color':jobName != '请选择岗位角色'}">{{ jobName }}</span>
					</li>
					<li class="item arrows-right" v-hasPermi="['app:place:index']" style="border: 0"
						@click="$refs.place.showPlace(placeIds)">
						<span class="item-left">场地权限</span><span class="item-right"
							:class="{'font-color':placeIds.length}">
							{{placeIds.length? `已选择${placeIds.length}个场地`: "请选择场地权限"}}
						</span>
					</li>
					<li class="synchronous-new-group">
						<span class="synchronous-new-group-text">自动同步授权新建的场地</span>
				...
				<view class="autoAccount-content" v-show="ledger">
					<view class="item-top" @click="$refs.picker.showMember(separateBillsId)">
		...
		methods: {
			...
			inputMethod(value) {
				...
				if (value.startsWith('zt')) {
					...
					_this.sonAccountInfo.username = 'zt' + value;
				}
			},
		},
	};
