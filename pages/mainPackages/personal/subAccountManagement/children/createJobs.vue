<template>
	<view class="createJobs">
		...
		<view class="create-role home-family">
			<u-cell title="岗位名称">
				<template slot="value">
					<u-input v-model="jobName" placeholder="请输入岗位名称" border="none" inputAlign="right"
						:readonly="!editJobs ? true : false" />
				</template>
			</u-cell>
			...
			<view class="container">
				<view class="container-content">
					<view class="all-check">
						<u-checkbox-group>
							<u-checkbox :checked="deptNodeAll" :size="40" labelSize="25" iconSize="25" label="全选/全不选"
								@change="handleCheckedTreeNodeAll($event)" 
								activeColor="#5241FF">
							</u-checkbox>
						</u-checkbox-group>
					</view>
					<DaTreeVue2 ref="DaTreeRef" :data="menuOptions" labelField="title" valueField="menuId" showCheckbox
						:defaultCheckedKeys="selectTreeData" @change="handleTreeChange" />
				</view>
			</view>
			<!-- bottom -->
			<!-- @click="addOredit" -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			//反选-角色权限
			roleCheck() {
				subAccountController.getRoleModuleById({
					roleId: this.roleId
				}).then(res => {
					if (res.code == 200) {
						this.$nextTick(() => {
			...
			async deleteJob() {
				this.$modal("确定要删除岗位角色?").then(() => {
					subAccountController.deleteSysRole({
						roleId: this.roleId
					}).then(res => {
						if (res.code == 200) {
							this.$toast("删除成功!")
