<template>
	<view class="couple-detail">
		...
		<view class="complaint-details">
			...
			<!-- 选择处理方式 微信-->
			<view class="details-list" v-if="coupleDetail.sourceType == 1&&coupleDetail.conductorState==1">
				...
				<view class="reason-wrapper">
					...
					<textarea placeholder="请输入具体协商情况,至少10个字 ，此留言将直接同步消费者。" v-model="responseContent"
						class="area-content" :disabled="pickerReason != 6"></textarea>
					<view class="upload">
						<u-upload name="wxImg" :previewFullImage="true" :fileList="fileList" @afterRead="afterRead"
							@delete="deleteRead" :maxCount="4" width="128" height="128">
							<view class="upload-img">
								<view class="main-wrapper">
									<view>
										<xls-image :src="`${$baseUrl}appV4/image/couple/camera.png`" alt=""
											class="image" />
									</view>
									<view>上传图片</view>
								</view>
			...
			<!-- 处理方式 支付宝 -->
			<view class="details-list" v-if="coupleDetail.sourceType == 3 && coupleDetail.conductorState == 1">
				<view class="title-type">选择处理方式:</view>
				<view class="reason-wrapper">
					...
					<textarea placeholder="请输入具体协商情况,至少10个字 ，此留言将直接同步消费者。" v-model="responseContent"
						class="area-content" :disabled="pickerReasonZFB != 5"></textarea>
				</view>
				<view class="footer-btn-wrapper">
			...
			<view class="details-list">
				<view class="image-content" v-if="coupleDetail.sourceType == 2">
					<view class="txt-detail">{{ coupleDetail.problemDescription }}</view>
					<img :src="coupleDetail.problemImg" alt="" class="img"
						@click="imgView([coupleDetail.problemImg], 0)" v-if="coupleDetail.problemImg" />
				</view>
				<view class="list-content">
					<view class="info">
						<span class="field">反馈类型</span>
						<span
							class="value">{{coupleDetail.feedbackType == 1?'问题':coupleDetail.feedbackType==2?'意见':'投诉'}}</span>
					</view>
					<view class="info">
					...
					<view class="operate" v-if="coupleDetail.orderNo">
						...
						<span class="send-coins" v-if="0">派发福利</span>
					</view>
					<view class="info">
						<span class="field">设备类型</span><span class="value" v-html="coupleDetail.deviceTypeName"></span>
					</view>
					<view class="info">
						<span class="field">当前设备状态</span><span class="value"
							v-html="coupleDetail.onlineState ? '在线' : '离线'"></span>
					</view>
					<view class="info">
			...
			<!-- 系统回复 -->
			<view class="reply" v-if="coupleDetail.conductorState == 1 && coupleDetail.sourceType == 2">
			...
			<view class="bottom-wrapper" v-if="coupleDetail.conductorState == 1 && coupleDetail.sourceType == 2">
			...
			<view class="egotiate-history" v-if="coupleDetail.sourceType==2&&historyList.length">
				...
				<view v-for="(item, index) in historyList" :key="index">
					...
					<!-- 回复 -->
					<view class="content-info" v-if="item.replyContent">
						<view class="left">
							<span class="head"><span class="point"></span></span>
							<span class="line"></span>
						</view>
			...
			<!-- 普通单回复 -->
			<view class="egotiate-history"
				v-if="coupleDetail.sourceType == 2 &&!historyList.length && coupleDetail.replyContent">
				<view class="big-title">协商历史</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				reasonList: [{
						id: 1,
						resTitle: "已协商处理",
						resMsg: "您的反馈已处理完毕，请于微信支付订单进行撤诉操作，感谢您的配合。",
					},
					{
						id: 2,
						resTitle: "已协商退款",
						resMsg: "您好,该投诉已双方协商并线下退款，请您于微信支付订单进行撤诉操作，感谢您的配合。",
					},
					{
						id: 3,
						resTitle: "已协商并派发福利",
						resMsg: "您好,该投诉已双方协商处理并线下派发福利，请您于微信支付订单进行撤诉操作，感谢您的配合。",
					},
					{
						id: 4,
						resTitle: "无法联系到消费者",
						resMsg: "您好！已多次联系，均无法联系到您，给您带来不便，敬请见谅。如有需协助处理，请拨打客服电话。",
					},
					{
						id: 5,
						resTitle: "无联系方式",
						resMsg: "您好！无法获取您的联系方式，给您带来不便，敬请见谅。如有需协助处理，请拨打客服电话。",
					},
					{
						id: 7,
						resTitle: "常用回复",
						resMsg: "您好！申诉问题已经解决，如有问题请继续申诉，资金预计10分钟左右到账。",
					},
					{
						id: 6,
						resTitle: "其他",
		...
		methods: {
			//查询详情
			async getDetail() {
				let res = await appealController.getRepresentationDetail({
					id: this.id
				});
				if (res.code == 200) {
			...
			async replyWx() {
				...
				let res = await appealController.replyFeed(params);
				if (res.code == 200) {
					this.$modal("回复成功", {
							title: "温馨提示",
							confirmColor: "#5241FF",
			...
			async afterRead(file) {
				...
				let base64 = await blobUrlToBase64(file.file.url)
				const params = {
					buf: base64,
					filePath: file.file.name,
				}
				let res = await appealController.setComplaintsImg(params);
				if (res.code == 200) {
					const {
						media_id
					} = JSON.parse(res.data);
					...
					this.responseImages.push(media_id);
				}
			},
			deleteRead(event) {
			...
			async closeReply() {
				...
				this.$modal("确定已经协商完成吗？",{
					...
					.then(() => {
						...
						appealController.completeComplaints({
							complaintId: this.coupleDetail.complaintId,
			...
			goTo(route) {
				if (route === 'remote') {
					this.$goTo("/pages/mainPackages/home/remote/operate", "navigateTo", {
						railNumber: this.coupleDetail.railNumber,
						shippingSpace: this.coupleDetail.shippingSpace,
						deviceNumber: this.coupleDetail.deviceNumber,
						deviceTypeId: this.coupleDetail.deviceType
					})
					return
				} 
				if(route === 'order') {
					this.$goTo('/pages/mainPackages/home/order/orderDetail', 'navigateTo', {
						orderId: this.coupleDetail.orderNo
					})
				}
			},
