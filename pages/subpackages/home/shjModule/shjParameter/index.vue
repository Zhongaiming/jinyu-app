  ...
  <view>
    <view id="parameterSetting">
      ...
      <view class="setting">
        ...
        <view
          class="right"
          @click="$refs.typeList.openBusiness(business.businessTypeId)"
        >
      ...
      <view class="item" v-show="shoppingCartMode">
        ...
        <view class="text-input">
          <input
            class="padding-r-3"
            placeholder="请输入购买上限, 购买上限为1-20"
            type="tel"
            v-model="business.purchaseCeiling"
            @blur="blurCeiling"
          />
        </view>
      </view>
      ...
      <view
        class="btn"
        @click="handleUpdata"
        v-hasPermi="['app:shj:params:edit']"
      >
        保存
      </view>
...
export default {
  ...
  methods: {
    ...
    async getType() {
      ...
      let res = await api.businessOne({ deviceNumber });
      if (res.data.code == 0) {
