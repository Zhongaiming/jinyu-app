<template>
  <view class="shj-grounding">
    ...
    <view class="container-content">
      ...
      <view
        class="exhibition-mode"
        v-hasPermi="['app:shj:grounding:type']"
        @click="$refs.type.showType = true"
      >
...
export default {
  name: "shjGrounding",
  // components: { ShowType, EditRail, BatchSetting },
  data() {
    return {
      deviceNumber: "",
      railList: [],
  ...
  methods: {
    ...
    async getDetail() {
      ...
      let res = await api.replenishmentDetails({ deviceNumber });
      if (res.data.code == 0) {
        this.railList = res.data.data.list;
        this.allRailList = res.data.data.list;
