...
<script>
	...
	export default {
		...
		methods: {
			async getAreaNumber() {
				let province = await merchantController.getProvinceList();
				if (province.code == 200) {
					let city = await merchantController.getCityList({
						provinceCode: province.data[0].addressCode
					})
					if (city.code == 200) {
						this.optionList = [province.data, city.data]
					}
				}
				...
				// });
				// if (res.code === 200) {
				// 	this.optionList = [province.data, city.data, three.data]
				// 	console.log('结果数据', this.optionList)
				// }
			},
			changeHandler(params) {
				...
				uni.$u.debounce(async function() {
					...
					if (columnIndex === 0) {
						// 滚动第一第二列
						let city = await merchantController.getCityList({
							provinceCode: value[0].addressCode
						})
						// let city = await merchantController.getAreaNumber({
