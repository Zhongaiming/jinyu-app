<template>
  <view class="zb-dropdown-menu">
    <view class="zb-dropdown-menu__bar zb-dropdown-menu__bar--opened" :style="[barStyle]"ref="bar">
      <view class="zb-dropdown-menu__item" @click.stop="toggleItem(index,item)" :key="index" v-for="(item,index) in childrenList">
...
export default {
  ...
  methods: {
    ...
    changeStatus(item, param1, param2, status) {
      ...
      this.watchTimer = setTimeout(() => {
        // item[param2] = status
        this.$set(item,param2,status)}, status ? 20 : 200)
    },
    toggleItem(index) {
      for (let i = 0; i < this.childrenList.length; i++) {
        let item = this.childrenList[i]
