	...
	export default {
		...
		methods: {
			getSysInfo() {
				...
				this.windowHeightXls = sysInfo.windowHeight
				this.windowWidthXls = sysInfo.windowWidth
				...
				view.boundingClientRect(rect => {
					...
					this.y = this.old.y
					this.$nextTick(res => {
						this.x = this.windowWidthXls - this.btnWidth - this.rightPx
						this.y = this.windowHeightXls - this.btnHeight - this.bottomPx
					})
				}).exec()
			},
			//移动按钮
			...
			touchend(e) {
				if (this.canDocking && this.old.x) {
					this.x = this.old.x
					this.y = this.old.y
					let bWidth = (this.windowHeightXls - this.btnWidth) / 2
					if (this.x < 0 || (this.x > 0 && this.x <= bWidth)) {
						...
						this.$nextTick(res => {
							this.x = this.windowHeightXls - this.btnWidth
						})
					}
					this.isRemove = false
				}
...
<style scoped>
	.movable-view {
		width: 100rpx;
		height: 100rpx;
		background: linear-gradient(360deg, #5241ff 0%, #b9b0ff 100%);
		box-shadow: 0px 4rpx 12rpx 0px #b9b0ff;
		border-radius: 50rpx;
		color: #FFFFFF;
		font-size: 30rpx;
		touch-action: none;
